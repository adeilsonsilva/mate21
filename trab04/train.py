...
MODEL_FILENAME = OUTPUT_DIR + "/model.ckpt"
PLOT_DATA_OUTPUT = OUTPUT_DIR + "/performance.csv"
...
def create_tf_graph():
    ...
    with graph.as_default():
        X = tf.placeholder(tf.float32, shape = (None, common_data.IMG_HEIGHT, common_data.IMG_WIDTH, common_data.NUM_CHANNELS), name="net_input")
        ...
        out = tf.layers.conv2d(out, 128, (3, 3), (1, 1), padding='same')
        print("Conv. Layer 3: {}".format(out.shape))
        out = tf.layers.max_pooling2d(out, (2, 2), (2, 2), padding='same')
        out = tf.layers.batch_normalization(out, training=is_training)
        # print("Max Pooling Layer 3: {}".format(out.shape))
        # out = tf.layers.conv2d(out, 256, (3, 3), (1, 1), padding='valid')
        # print("Conv. Layer 4: {}".format(out.shape))
        # out = tf.layers.max_pooling2d(out, (2, 2), (2, 2), padding='valid')
        # out = tf.layers.batch_normalization(out, training=is_training)
        # print("Max Pooling Layer 4: {}".format(out.shape))
        ...
        with tf.control_dependencies(update_ops):
            train_op = tf.train.MomentumOptimizer(learning_rate=learning_rate, momentum=0.55).minimize(loss)
            # train_op = tf.train.AdamOptimizer(learning_rate=learning_rate).minimize(loss)
            # train_op = tf.train.GradientDescentOptimizer(learning_rate=learning_rate).minimize(loss)

        result = tf.argmax(out, 1, name="net_output")
        correct = tf.reduce_sum(tf.cast(tf.equal(result, y), tf.float32))

        print("================================================")

    return graph, X, y, train_op, loss, correct, learning_rate, is_training, result, saver
...
def train(training_images, validation_images, path, l_rate = common_data.L_RATE, max_epochs = common_data.MAX_EPOCHS):
    ...
    my_graph, tf_X, tf_y, tf_optimizer, tf_loss, tf_correct, tf_lr, tf_isTraining, tf_result, tf_saver = create_tf_graph()
    ...
    with tf.Session(graph = my_graph) as session:
        ...
        for epoch in range(max_epochs):
            ...
            if should_stop:
                ...
                break
        tf_saver.save(session, MODEL_FILENAME)
    out_plot.close()
